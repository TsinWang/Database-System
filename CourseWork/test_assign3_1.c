  ...
  for(i = 0; i < 1000; i++)
    {
      ...
      RID rid = rids[pos];
     printf("\n----------------- Test get 1 record no. %d", i);
      TEST_CHECK(getRecord(table, rid, r));
        printf("\n----------------- Test get 2 record no. %d", i);
...
void testScansTwo (void)
{
  ...
  createRecord(&r, schema);
  TEST_CHECK(startScan(table, sc, sel));
    printf("\n[ ~~~~Test value is %s", serializeValue(sel->expr.op->args[0]->expr.cons));
  ...
  MAKE_CONS(left, stringToValue("sffff"));
    //MAKE_CONS(left, stringToValue("i5"));
  ...
  createRecord(&r, schema);
  TEST_CHECK(startScan(table, sc, sel));
 printf("\n[ ~~~~Test value is %d", sel->expr.op->args[1]->expr.attrRef);
  while((rc = next(sc, r)) == RC_OK)
  {
...
{
//  printf("\n in.a = %d, in.b = %s, int.c = %d", in.a, in.b, in.c);
